...
public class MixedGranularityTraces extends Metric {

	private Map<String, Integer> eventCount = new HashMap<>();

	public MixedGranularityTraces(String name, String dimension, String logLevel) {
	...
	public void calculateScore() {
		if (getEventList().size() > 0) {
			double score = 1;
			Map<String, List<Double>> expectedTraceGranularity = new HashMap<>();
			for (Entry<Error, List<Error>> e : getErrorList().entrySet()) {
				...
				double[] nullRatio = { 1, (double) 11 / 12, (double) 353.25 / 365.25, (double) 23 / 24,
						(double) 59 / 60, (double) 59 / 60, (double) 999 / 1000 };
				double[] expectedPrecision = new double[7];
				expectedPrecision[max] = (double) nullRatio[max];
				...
				expectedTraceGranularity.put((String) e.getKey().getErrorObject(), expectedPrecisionList);
			}
			for (Entry<Error, List<Error>> e : getErrorList().entrySet()) {
				double partialScore = 0;
				if (!e.getKey().isWhitelisted()) {
					for (int i = 0; i < 7; i++) {
						partialScore = partialScore
								+ Math.abs((Double) ((Error) e.getValue().toArray()[i]).getErrorObject()
										- expectedTraceGranularity.get(e.getKey().getErrorObject()).get(i));
					}
				}
				partialScore = partialScore * eventCount.get(e.getKey().getErrorObject()) / getEventList().size();
	...
	public void detectErrors() {
		...
		Collections.sort(getEventList(), new TraceSorter());
		...
		while (lit.hasNext()) {
			...
			if (!lit.hasNext() || !XConceptExtension.instance()
				...
				eventCount.put(
						XConceptExtension.instance().extractName(getEventList().get(lit.previousIndex()).getTrace()),
						IntStream.of(precision).sum());
				for (int i = 0; i < 7; i++) {
					Error error = new Error((double) precision[i] / eventCount.get(XConceptExtension.instance()
							.extractName(getEventList().get(lit.previousIndex()).getTrace())), false);
					errorList.add(error);
				}
	...
	public void createErrorTableModel() {
		...
		errorTableModel.addColumn("Trace");
